...
public class MCPTool {
    private static final String APK_SOURCE_PATH = RedisKeyProperties.getPropertyValue("apk_source_path");
    private static final String APK_CHANNEL_PATH = RedisKeyProperties.getPropertyValue("apk_channel_path");
    /**
     * 数据结构体的签名标记
     */
...
//		Long version1 = (Long) versions[0];
    ...
    public static String writeChannelAndGetPath(String channelNumber) throws Exception {
        String[] args = {};
        args = ("-path " + APK_SOURCE_PATH + " -outdir " + APK_CHANNEL_PATH + " -contents " + channelNumber + " -password 12345678").split(" ");
                        ...
                        if (contents == null || contents.length == 0) { // 读取数据；
                            ...
                            for (String content : contents) {
                                ...
                                if (nioTransferCopy(path, target)) {
                                    ...
                                    System.out.println("生成的文件路径为： : " + target.getPath());
